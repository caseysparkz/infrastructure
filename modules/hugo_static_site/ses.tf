...
resource "aws_ses_domain_dkim" "root_domain" { domain = aws_ses_domain_identity.root_domain.id }

resource "aws_ses_email_identity" "default_sender" { email = local.email_headers["default_sender"] }

resource "aws_ses_email_identity" "default_recipient" { email = local.email_headers["default_recipient"] }
